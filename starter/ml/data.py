...
def process_data(
    X, categorical_features: list=None, label=None, training=True, encoder=None, lb=None, mapping_labels=None
):
    ...
    if label is not None:
        X[label] = X[label].map(mapping_labels) #inpute labels according to mapping_labels dictionary
        y = X[label]
    ...
    if training is True:
        encoder = OneHotEncoder(handle_unknown="ignore", sparse=False)  # for sklearn 1.0.2; for newer versions use sparse_output=False
